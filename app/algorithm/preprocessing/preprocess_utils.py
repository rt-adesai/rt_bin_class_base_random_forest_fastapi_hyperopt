...
def get_cat_and_num_vars_lists(data_schema):      
    cat_vars, num_vars = [], []
    attributes = data_schema["inputDatasets"]["binaryClassificationBaseMainInput"]["predictorFields"]   
...
def get_preprocess_params(data, data_schema, model_cfg): 
    ...
    pp_params["id_field"] = data_schema["inputDatasets"]["binaryClassificationBaseMainInput"]["idField"]   
    
    # set the target attribute
    pp_params["target_attr_name"] = data_schema["inputDatasets"]["binaryClassificationBaseMainInput"]["targetField"]   
    
    
    # set the target class (class of interest, to be set to 1)
    pp_params["target_class"] = data_schema["inputDatasets"]["binaryClassificationBaseMainInput"]["targetClass"]   
